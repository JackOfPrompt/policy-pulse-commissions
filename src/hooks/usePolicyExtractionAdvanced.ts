...
export function usePolicyExtractionAdvanced() {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [data, setData] = useState<PolicyExtractionResult | null>(null);

  const extractPolicy = useCallback(async (
    filePath: string, 
    productType?: string,
    saveTraining: boolean = false,
    policyId?: string
  ) => {
    try {
      setLoading(true);
  ...
  const finalizePolicyData = useCallback(async (
    policyId: string, 
    ...
    if (policyIndex !== -1) {
      const originalData = existing[policyIndex].extractedData;
