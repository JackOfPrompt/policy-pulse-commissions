...
export default function ManageInsuranceProviders() {
  ...
  const handleStatusToggle = async (providerId: string, currentStatus: string) => {
    try {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          try {
            ...
            const { data: existingProvider } = await supabase
              .from('master_insurance_providers')
              .select('provider_id')
              .eq('provider_code', row.provider_code)
              .single();
            ...
            if (existingProvider) {
              ...
              const { error } = await supabase
                .from('master_insurance_providers')
                .update(providerData)
                .eq('provider_id', existingProvider.provider_id);
  ...
  const downloadErrorReport = () => {
    ...
    const csvContent = [
      ...
      ...importErrors.map(error => [
        ...
        error.data.provider_code || '',
        `"${(error.data.provider_name || '').replace(/"/g, '""')}"`,
        error.data.irda_license_number || '',
        error.data.contact_email || '',
        `"${JSON.stringify(error.data).replace(/"/g, '""')}"` // Full row data as JSON
      ].join(','))
    ].join('\n');
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground">Manage insurance provider master data</p>
          </div>
        </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                   ...
                   <div className="space-y-3">
                     ...
                     <div className="flex items-center space-x-4">
                       {(logoPreview || editingProvider?.logo_file_path) && (
                         <div className="relative">
                           <img 
                             src={logoPreview || editingProvider?.logo_file_path} 
                             alt="Logo preview" 
            ...
            <TableBody>
              {loading ? (
                ...
                providers.map((provider) => (
                  <TableRow key={provider.provider_id} className="hover:bg-muted/50 cursor-pointer">
                     <TableCell>
                       {provider.logo_file_path ? (
                         <img 
                           src={provider.logo_file_path} 
                           alt={`${provider.provider_name} logo`}
                           className="h-8 w-8 object-contain rounded bg-white border"
                           onError={(e) => {
                    ...
                    <TableCell className="font-medium">{provider.provider_code}</TableCell>
                    <TableCell>{provider.provider_name}</TableCell>
                    <TableCell className="text-muted-foreground">{provider.trade_name || '-'}</TableCell>
                    <TableCell>
                      {provider.provider_type && (
                        <Badge variant="outline">{provider.provider_type}</Badge>
                      )}
                    </TableCell>
                    <TableCell className="font-mono text-sm">{provider.irda_license_number}</TableCell>
                    <TableCell>{format(new Date(provider.irda_license_valid_till), 'MMM dd, yyyy')}</TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <div className="flex items-center space-x-2">
                        <Switch
                          ...
                          onCheckedChange={() => handleStatusToggle(provider.provider_id, provider.status || 'Inactive')}
                        />
