-- Phase 1: Schema Consolidation - Drop Duplicates and Merge Critical Tables
...
-- Step 3: Merge employees tables - Keep tenant_employees as the primary table
-- First, migrate data from employees to tenant_employees if needed
INSERT INTO public.tenant_employees (
  tenant_id, name, email, phone, status, created_at
)
SELECT 
  tenant_id, 
  name, 
...
WHERE NOT EXISTS (
  ...
  WHERE te.email = employees.email AND te.tenant_id = employees.tenant_id
)
...
-- Migrate data from tenant_organizations to organizations
...
-- Migrate data from orgs table if it exists
DO $$
BEGIN
...
-- Migrate data from accounts to finance_accounts if needed
...
-- Migrate payout data to finance_payouts
