...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_tbd_tenant_id') THEN
    EXECUTE 'ALTER TABLE public.tenant_branch_departments ADD CONSTRAINT fk_tbd_tenant_id FOREIGN KEY (tenant_id) REFERENCES public.tenants(tenant_id) ON DELETE CASCADE';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_tbd_branch_id') THEN
    EXECUTE 'ALTER TABLE public.tenant_branch_departments ADD CONSTRAINT fk_tbd_branch_id FOREIGN KEY (branch_id) REFERENCES public.branches(branch_id) ON DELETE CASCADE';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_tbd_department_id') THEN
    EXECUTE 'ALTER TABLE public.tenant_branch_departments ADD CONSTRAINT fk_tbd_department_id FOREIGN KEY (department_id) REFERENCES public.departments(department_id) ON DELETE CASCADE';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_eda_tenant_id') THEN
    EXECUTE 'ALTER TABLE public.employee_department_assignments ADD CONSTRAINT fk_eda_tenant_id FOREIGN KEY (tenant_id) REFERENCES public.tenants(tenant_id) ON DELETE CASCADE';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_eda_branch_id') THEN
    EXECUTE 'ALTER TABLE public.employee_department_assignments ADD CONSTRAINT fk_eda_branch_id FOREIGN KEY (branch_id) REFERENCES public.branches(branch_id) ON DELETE CASCADE';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_eda_department_id') THEN
    EXECUTE 'ALTER TABLE public.employee_department_assignments ADD CONSTRAINT fk_eda_department_id FOREIGN KEY (department_id) REFERENCES public.departments(department_id) ON DELETE CASCADE';
  END IF;
  ...
  ) AND NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_eda_employee_id') THEN
    EXECUTE 'ALTER TABLE public.employee_department_assignments ADD CONSTRAINT fk_eda_employee_id FOREIGN KEY (employee_id) REFERENCES public.employees(id) ON DELETE CASCADE';
  END IF;
END$$;

-- 4) Validation triggers instead of CHECK constraints
CREATE OR REPLACE FUNCTION public.validate_tenant_branch_departments()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE b.branch_id = NEW.branch_id AND b.tenant_id = NEW.tenant_id
  ) THEN
...
CREATE OR REPLACE FUNCTION public.validate_employee_department_assignments()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
DECLARE
  ...
  v_has_emp_tenant boolean;
BEGIN
  v_has_employees := to_regclass('public.employees') IS NOT NULL;
  v_has_emp_tenant := v_has_employees AND EXISTS (
    SELECT 1 FROM information_schema.columns
  ...
  -- Department must be assigned to tenant-branch and active
  IF NOT EXISTS (
    SELECT 1 FROM public.tenant_branch_departments tbd
    WHERE tbd.tenant_id = NEW.tenant_id
      AND tbd.branch_id = NEW.branch_id
      AND tbd.department_id = NEW.department_id
      AND tbd.is_active = true
  ) THEN
    RAISE EXCEPTION 'Department % is not assigned to tenant % branch % or is inactive', NEW.department_id, NEW.tenant_id, NEW.branch_id;
  END IF;

  -- Employee must belong to tenant if employees table supports it
  IF v_has_emp_tenant THEN
    IF NOT EXISTS (
      SELECT 1 FROM public.employees e
      WHERE e.id = NEW.employee_id AND e.tenant_id = NEW.tenant_id
    ) THEN
      RAISE EXCEPTION 'Employee % does not belong to tenant %', NEW.employee_id, NEW.tenant_id;
    END IF;
  END IF;
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='tbd_validate_before_write') THEN
    EXECUTE 'CREATE TRIGGER tbd_validate_before_write
      BEFORE INSERT OR UPDATE ON public.tenant_branch_departments
      FOR EACH ROW EXECUTE FUNCTION public.validate_tenant_branch_departments()';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='eda_validate_before_write') THEN
    EXECUTE 'CREATE TRIGGER eda_validate_before_write
      BEFORE INSERT OR UPDATE ON public.employee_department_assignments
      FOR EACH ROW EXECUTE FUNCTION public.validate_employee_department_assignments()';
  END IF;
END$$;

-- 5) Keep updated_at triggers (idempotent)
DO $$ BEGIN
  IF NOT EXISTS (
...
-- 6) Enable RLS and policies (idempotent)
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname='public' AND tablename='tenant_branch_departments' AND policyname='tbd_select_own_tenant'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='tenant_branch_departments' AND policyname='tbd_insert_own_tenant'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='tenant_branch_departments' AND policyname='tbd_update_own_tenant'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='tenant_branch_departments' AND policyname='tbd_delete_own_tenant'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='employee_department_assignments' AND policyname='eda_select_own_tenant'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='employee_department_assignments' AND policyname='eda_insert_own_tenant'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='employee_department_assignments' AND policyname='eda_update_own_tenant'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='employee_department_assignments' AND policyname='eda_delete_own_tenant'
  ) THEN
...
-- employee_roles policies
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='employee_roles' AND policyname='er_select_all_auth'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='employee_roles' AND policyname='er_write_admin_only'
  ) THEN
...
-- 7) Business logic functions (idempotent)
...
BEGIN
  ...
  FOREACH dept_id IN ARRAY p_department_ids LOOP
    ...
    ON CONFLICT (tenant_id, branch_id, department_id) DO UPDATE
      SET is_active = true,
...
DECLARE
  assignment_id uuid;
BEGIN
  -- validation handled by trigger; proceed to upsert
  ...
  ON CONFLICT (tenant_id, employee_id, department_id, branch_id) DO UPDATE SET
...
AS $$
DECLARE
  v_has_employees boolean := to_regclass('public.employees') IS NOT NULL;
  v_name_col text;
  v_code_col text;
  sql text;
BEGIN
  ...
  sql := 'SELECT eda.id, '
      || (CASE WHEN v_has_employees THEN 'e.id' ELSE 'eda.employee_id' END) || ', '
      || (CASE WHEN v_has_employees AND v_name_col IS NOT NULL THEN 'e.' || quote_ident(v_name_col) ELSE 'NULL::varchar' END) || ', '
      || (CASE WHEN v_has_employees AND v_code_col IS NOT NULL THEN 'e.' || quote_ident(v_code_col) ELSE 'NULL::varchar' END) || ', '
      || 'b.branch_name, d.name, er.name, eda.is_primary, eda.start_date, eda.end_date '
