-- Update the commission calculation function to handle external source reporting employee commission correctly
...
CREATE OR REPLACE FUNCTION public.calculate_enhanced_comprehensive_commission_report(p_org_id uuid DEFAULT NULL::uuid, p_policy_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(policy_id uuid, policy_number text, customer_name text, product_category text, product_name text, plan_name text, provider text, premium_amount numeric, source_type text, employee_id uuid, employee_name text, employee_code text, agent_id uuid, agent_name text, agent_code text, misp_id uuid, misp_name text, channel_partner_name text, grid_table text, grid_id uuid, base_commission_rate numeric, reward_commission_rate numeric, bonus_commission_rate numeric, total_commission_rate numeric, insurer_commission numeric, agent_commission numeric, misp_commission numeric, employee_commission numeric, reporting_employee_commission numeric, broker_share numeric, commission_status text, calc_date timestamp with time zone, policy_start_date date)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  LOOP
    ...
    IF LOWER(policy_rec.product_category) = 'life' THEN
      ...
      WHERE lpg.org_id = p_org_id
        ...
        AND (
          ...
          OR (lpg.provider_id IS NULL AND LOWER(lpg.provider) = LOWER(policy_rec.provider))
        )
    ...
    ELSIF LOWER(policy_rec.product_category) = 'health' THEN
      ...
      WHERE hpg.org_id = p_org_id
        ...
        AND (
          ...
          OR (hpg.provider_id IS NULL AND LOWER(hpg.provider) = LOWER(policy_rec.provider))
        )
    ...
    ELSIF LOWER(policy_rec.product_category) = 'motor' THEN
      -- Check motor_payout_grid with improved matching
      SELECT 
        mpg.id as grid_id,
      ...
      WHERE mpg.org_id = p_org_id
        ...
        AND (
          ...
          OR (mpg.provider_id IS NULL AND LOWER(mpg.provider) = LOWER(policy_rec.provider))
        )
    ...
    IF grid_rec IS NOT NULL THEN
      ...
      -- Distribute based on source type with updated logic for external sources
      IF policy_rec.source_type = 'external' AND policy_rec.agent_id IS NOT NULL THEN
        ...
        -- NEW LOGIC: For external sources, reporting employee commission = insurer commission - agent commission
        -- Broker share = 0
        calculated_reporting_employee_commission := calculated_insurer_commission - calculated_agent_commission;
        calculated_broker_share := 0;
      ...
      ELSIF policy_rec.source_type = 'external' AND policy_rec.misp_id IS NOT NULL THEN
        ...
        -- NEW LOGIC: For external sources, reporting employee commission = insurer commission - misp commission
        -- Broker share = 0
        calculated_reporting_employee_commission := calculated_insurer_commission - calculated_misp_commission;
        calculated_broker_share := 0;
