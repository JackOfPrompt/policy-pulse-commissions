-- Fix the calculate_comprehensive_commission_report function to use correct column names
CREATE OR REPLACE FUNCTION public.calculate_comprehensive_commission_report(p_org_id uuid DEFAULT NULL::uuid, p_policy_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(policy_id uuid, policy_number text, product_category text, product_name text, plan_name text, provider text, source_type text, grid_table text, grid_id uuid, commission_rate numeric, reward_rate numeric, insurer_commission numeric, agent_commission numeric, misp_commission numeric, employee_commission numeric, broker_share numeric, calc_date timestamp with time zone)
 LANGUAGE sql
 SECURITY DEFINER
...
grid_match AS (
  -- Life matching using correct column names
  SELECT 
    b.policy_id,
    lpg.id AS grid_id,
  ...
  UNION ALL
  -- Health matching with correct column names
  SELECT 
    b.policy_id,
    hpg.id AS grid_id,
  ...
  UNION ALL
  -- Motor matching with correct column names
  SELECT 
    b.policy_id,
    mpg.id AS grid_id,
