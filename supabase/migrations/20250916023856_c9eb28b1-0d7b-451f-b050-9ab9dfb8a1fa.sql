-- Fix security issues and update functions with standardized field names

-- 1. Drop the view that has security issues and recreate without SECURITY DEFINER
DROP VIEW IF EXISTS commission_grid_unified;

-- 2. Update the comprehensive commission calculation function to use standardized field names
...
-- 3. Update other functions to use standardized field names and fix search path
...
BEGIN
  ...
  -- Match Commission Grid using standardized field names
