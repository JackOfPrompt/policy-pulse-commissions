-- Retry migration with idempotent index creation (IF NOT EXISTS)
...
-- 6) Trigger function (before trigger)
...
-- 7) Unique index IF NOT EXISTS
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='subscription_plans') THEN
    EXECUTE 'CREATE UNIQUE INDEX IF NOT EXISTS subscription_plans_plan_code_key ON public.subscription_plans (plan_code)';
  END IF;
END $$;
