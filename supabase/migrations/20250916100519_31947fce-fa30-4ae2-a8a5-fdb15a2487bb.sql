-- Fix commission calculation function to properly match policies with grids
...
BEGIN
  ...
  grid_match AS (
    ...
    JOIN life_payout_grid lpg 
     ...
     AND lpg.provider = b.provider  -- Match by provider text, not provider_id
     AND lower(lpg.product_type) = lower(b.product_category)  -- Match category with product_type
    ...
    JOIN health_payout_grid hpg
     ...
     AND hpg.provider = b.provider  -- Match by provider text
     AND lower(hpg.product_type) = lower(b.product_category)  -- Match category with product_type
    ...
    JOIN motor_payout_grid mpg
     ...
     AND mpg.provider = b.provider  -- Match by provider text
     AND lower(mpg.product_type) = lower(b.product_category)  -- Match category with product_type
