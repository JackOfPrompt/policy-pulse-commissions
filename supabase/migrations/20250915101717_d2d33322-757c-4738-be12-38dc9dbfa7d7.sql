...
-- Drop and recreate the view without SECURITY DEFINER to avoid the security warning
DROP VIEW IF EXISTS policy_commission_report;

-- Create a non-security definer view for reporting
...
-- Add RLS to the view (will inherit from underlying tables)
ALTER VIEW policy_commission_report ENABLE ROW LEVEL SECURITY;