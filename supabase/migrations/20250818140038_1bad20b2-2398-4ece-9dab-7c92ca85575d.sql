...
-- Enable RLS and add policies tied to policy tenant
ALTER TABLE public.policy_map ENABLE ROW LEVEL SECURITY;

-- Select: users in same tenant as the mapped policy, or system_admins
CREATE POLICY IF NOT EXISTS policy_map_select
...
-- Insert: tenant_admins for their tenant (policy side)
CREATE POLICY IF NOT EXISTS policy_map_insert
ON public.policy_map
FOR INSERT
WITH CHECK (
  EXISTS (
...
-- Update/Delete: tenant_admins for their tenant (policy side)
CREATE POLICY IF NOT EXISTS policy_map_update
ON public.policy_map
FOR UPDATE
USING (
  EXISTS (
...
CREATE POLICY IF NOT EXISTS policy_map_delete
ON public.policy_map
FOR DELETE
USING (
  EXISTS (
...
-- 2) Views for reporting with tenant-aware filters via profiles + auth.uid()
...
-- Settlements (already tenant_uuid)
...
-- Revenue overview per tenant/date aggregating across sources
