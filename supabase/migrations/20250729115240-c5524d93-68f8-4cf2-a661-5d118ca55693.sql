...
CREATE POLICY "Admin and Finance can view all payouts" 
...
USING (
    EXISTS (
        ...
        AND ur.role IN ('admin', 'finance', 'accounts')
    )
);
...
CREATE POLICY "Admin and Finance can insert payouts" 
ON public.payout_transactions 
FOR INSERT 
WITH CHECK (
    EXISTS (
...
CREATE POLICY "Admin and Finance can update payouts" 
ON public.payout_transactions 
FOR UPDATE 
USING (
    EXISTS (
