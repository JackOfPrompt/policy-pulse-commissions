-- Fix the task_type enum casting issue in the enhanced_log_policy_status_change_with_tasks function
...
BEGIN
  IF OLD.policy_status IS DISTINCT FROM NEW.policy_status THEN
    ...
    IF NEW.policy_status = 'Underwriting' THEN
      ...
      -- Create the task - remove explicit enum casting
      INSERT INTO public.tasks (
        ...
        NEW.id,
        task_priority,
        'Review',  -- Let PostgreSQL handle the enum conversion
        task_due_date,
        'Open',
        NEW.status_updated_by
      );
