...
USING (EXISTS (
    ...
    AND (profiles.tenant_id::bigint = agents.tenant_id OR profiles.role = 'system_admin'::app_role)
));
...
USING (EXISTS (
    ...
    JOIN public.profiles p ON (p.tenant_id::bigint = a.tenant_id OR p.role = 'system_admin'::app_role)
