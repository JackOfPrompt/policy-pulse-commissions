-- Security Fix Migration: Enable RLS and Create Policies for Missing Tables
...
-- Skip views as they inherit security from underlying tables
-- Skip user management tables for now as they need special handling
...
-- Create RLS policies for commission tables
...
CREATE POLICY "Tenant users can view their roles" ON public.tenant_roles
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.profiles p 
      WHERE p.user_id = auth.uid() 
      AND (p.tenant_id = tenant_roles.tenant_id OR p.role = 'system_admin'::app_role)
    )
  );

CREATE POLICY "Tenant admins can manage their roles" ON public.tenant_roles
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM public.profiles p 
      WHERE p.user_id = auth.uid() 
...
-- Create RLS policies for workflow tasks if it exists
DO $$
BEGIN
