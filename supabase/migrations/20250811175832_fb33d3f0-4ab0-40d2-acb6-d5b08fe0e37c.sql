-- 1) Create tenants, subscription_plans, tenant_subscriptions with RLS and triggers
-- Note: uses existing helper functions is_admin(), current_user_tenant_id()

-- Create is_tenant_admin helper
...
create policy if not exists "Tenants are viewable by admin or matching tenant" on public.tenants
...
create policy if not exists "Only admins can insert tenants" on public.tenants
for insert to authenticated with check (is_admin(auth.uid()));

create policy if not exists "Admins or tenant admins can update their tenant" on public.tenants
...
create policy if not exists "Only admins can delete tenants" on public.tenants
for delete to authenticated using (is_admin(auth.uid()));

-- Policies for subscription_plans
create policy if not exists "Plans are viewable by authenticated" on public.subscription_plans
for select to authenticated using (true);

create policy if not exists "Only admins can insert plans" on public.subscription_plans
for insert to authenticated with check (is_admin(auth.uid()));

create policy if not exists "Only admins can update plans" on public.subscription_plans
for update to authenticated using (is_admin(auth.uid())) with check (is_admin(auth.uid()));

create policy if not exists "Only admins can delete plans" on public.subscription_plans
for delete to authenticated using (is_admin(auth.uid()));

-- Policies for tenant_subscriptions
create policy if not exists "Subscriptions viewable by admin or tenant admin of same tenant" on public.tenant_subscriptions
for select to authenticated using (
  is_admin(auth.uid()) OR (
...
create policy if not exists "Subscriptions insert by admin or tenant admin for own tenant" on public.tenant_subscriptions
...
create policy if not exists "Subscriptions update by admin or tenant admin for own tenant" on public.tenant_subscriptions
for update to authenticated using (
  is_admin(auth.uid()) OR (
    public.is_tenant_admin(auth.uid()) AND tenant_id = current_user_tenant_id()
  )
...
create policy if not exists "Subscriptions delete by admin or tenant admin for own tenant" on public.tenant_subscriptions
