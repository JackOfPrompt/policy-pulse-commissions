-- Phase 1: Schema Normalization Migration (Fixed)
...
-- 2. Populate status_master with common statuses FIRST
...
-- 3. Create channels table for all distribution channels
...
-- 4. Populate channels with standard distribution channels
...
-- 5. Create unified product_catalog table
...
-- 6. Add standardized audit columns to existing tables (WITHOUT foreign key constraints first)
...
-- 7. Create unified policies table structure
...
-- 8. Create policy_addons junction table
...
-- 9. Create unified commissions table
...
-- 10. NOW add foreign key constraints to the new columns
DO $$
BEGIN
  ...
  IF NOT EXISTS (
    ...
    FOREIGN KEY (status_id) REFERENCES public.status_master(status_id);
  END IF;
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'status_master_read_policy' AND tablename = 'status_master') THEN
    CREATE POLICY "status_master_read_policy" ON public.status_master
    FOR SELECT USING (auth.uid() IS NOT NULL);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'status_master_admin_policy' AND tablename = 'status_master') THEN
...
DROP TRIGGER IF EXISTS update_status_master_updated_at ON public.status_master;
