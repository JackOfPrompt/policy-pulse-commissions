-- Disable automatic commission calculation triggers
-- This allows policies to be inserted without automatic commission calculation
-- so that commissions can be handled separately

-- Drop existing triggers that automatically calculate commissions
DROP TRIGGER IF EXISTS auto_calculate_commission_trigger ON policies;
DROP TRIGGER IF EXISTS insert_policy_commission_trigger ON policies; 
DROP TRIGGER IF EXISTS recalc_policy_commission_trigger ON policies;

-- Create a manual function to calculate and insert commissions for a specific policy
...
-- Create a function to recalculate commissions for all policies when commission grids are updated  
