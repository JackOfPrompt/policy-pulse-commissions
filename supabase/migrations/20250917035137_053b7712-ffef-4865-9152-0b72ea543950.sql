-- Update the enhanced comprehensive commission report function to use base_percentage
...
BEGIN
  ...
  agent_tier_info AS (
    SELECT 
      ...
      COALESCE(a.override_percentage, a.base_percentage) as agent_base_percentage,
  ...
  SELECT 
    ...
    -- Agent commission calculation using base_percentage: insurer_commission * base_percentage
    CASE 
      WHEN ati.source_type = 'agent' AND ati.agent_id IS NOT NULL THEN 
        ati.total_insurer_commission * COALESCE(
          ...
          ati.agent_base_percentage,  -- Fallback to agent base_percentage
    ...
    -- Reporting employee commission: insurer_commission - agent_commission
    CASE 
      WHEN ati.source_type = 'agent' AND EXISTS (
        SELECT 1 FROM agents a 
        JOIN employees e ON e.id = a.employee_id 
        WHERE a.id = ati.agent_id
      ) THEN 
        ati.total_insurer_commission - (
          ati.total_insurer_commission * COALESCE(
            ati.tier_percentage,
      ...
      WHEN ati.source_type = 'misp' AND EXISTS (
        SELECT 1 FROM misps m 
        JOIN employees e ON e.id = m.employee_id 
        WHERE m.id = ati.misp_id
      ) THEN 
        ati.total_insurer_commission - (
          ati.total_insurer_commission * COALESCE(
            (SELECT percentage FROM misps WHERE id = ati.misp_id),
    ...
    CASE 
      WHEN ati.source_type = 'employee' THEN 0
      WHEN ati.source_type IN ('agent', 'misp') AND NOT EXISTS (
        SELECT 1 FROM agents a JOIN employees e ON e.id = a.employee_id WHERE a.id = ati.agent_id
        UNION
        SELECT 1 FROM misps m JOIN employees e ON e.id = m.employee_id WHERE m.id = ati.misp_id
      ) THEN ati.total_insurer_commission
      ELSE 0
    END AS broker_share,
    NOW() AS calc_date,
