-- Tenant-specific catalog and overrides
-- 1) Tenant Product Catalog (whitelist per tenant)
...
-- RLS: System admin full, tenant admin manage own, tenant members can view own
CREATE POLICY IF NOT EXISTS "Admin can manage tenant_product_catalog"
ON public.tenant_product_catalog
FOR ALL
USING (public.has_role(auth.uid(), 'admin'))
WITH CHECK (public.has_role(auth.uid(), 'admin'));

CREATE POLICY IF NOT EXISTS "Tenant admins can manage their product catalog"
ON public.tenant_product_catalog
FOR ALL
USING (public.is_tenant_admin_for(tenant_id))
WITH CHECK (public.is_tenant_admin_for(tenant_id));

CREATE POLICY IF NOT EXISTS "Tenant users can view their product catalog"
ON public.tenant_product_catalog
FOR SELECT
...
CREATE TABLE IF NOT EXISTS public.tenant_product_overrides (
  ...
  name text,
  description text,
  status text, -- overrides global status for display, still subject to base product visibility
  brochure_file_path text,
  available_addons jsonb,
  premium_frequency_options premium_frequency_enum[],
  premium_markup_rate numeric, -- percentage e.g., 0.10 = +10%
  premium_flat_add numeric,    -- absolute currency add-on
  effective_from date NOT NULL DEFAULT CURRENT_DATE,
  effective_to date,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
...
CREATE POLICY IF NOT EXISTS "Admin can manage tenant_product_overrides"
ON public.tenant_product_overrides
FOR ALL
USING (public.has_role(auth.uid(), 'admin'))
WITH CHECK (public.has_role(auth.uid(), 'admin'));

CREATE POLICY IF NOT EXISTS "Tenant admins can manage their product overrides"
ON public.tenant_product_overrides
FOR ALL
USING (public.is_tenant_admin_for(tenant_id))
WITH CHECK (public.is_tenant_admin_for(tenant_id));

CREATE POLICY IF NOT EXISTS "Tenant users can view their product overrides"
ON public.tenant_product_overrides
FOR SELECT
USING (tenant_id = ANY (public.current_user_tenant_ids()));
...
-- 3) Tenant Provider Catalog (whitelist per tenant)
...
CREATE POLICY IF NOT EXISTS "Admin can manage tenant_provider_catalog"
ON public.tenant_provider_catalog
FOR ALL
USING (public.has_role(auth.uid(), 'admin'))
WITH CHECK (public.has_role(auth.uid(), 'admin'));

CREATE POLICY IF NOT EXISTS "Tenant admins can manage their provider catalog"
ON public.tenant_provider_catalog
FOR ALL
USING (public.is_tenant_admin_for(tenant_id))
WITH CHECK (public.is_tenant_admin_for(tenant_id));

CREATE POLICY IF NOT EXISTS "Tenant users can view their provider catalog"
ON public.tenant_provider_catalog
FOR SELECT
USING (tenant_id = ANY (public.current_user_tenant_ids()));
...
-- 4) Resolved views
-- View to present tenant-resolved products applying overrides
...
-- View to present tenant-allowed providers
